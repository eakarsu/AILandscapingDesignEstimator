// Auto-generated feature route: crew-scheduling-agent
// Domain: field crew scheduler — Capability: optimizing crew assignments across projects (travel time, skill utilization)
// Mounted under: /api/crew-scheduling-agent
...
// POST /api/crew-scheduling-agent/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a field crew scheduler. Your task is optimizing crew assignments across projects (travel time, skill utilization). Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/crew-scheduling-agent/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/crew-scheduling-agent/:id — fetch one
...
// DELETE /api/crew-scheduling-agent/:id — discard a run
