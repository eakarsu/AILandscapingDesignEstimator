// Auto-generated feature route: customer-upsell-agent
// Domain: landscaping upsell specialist — Capability: recommending maintenance plans and seasonal services
// Mounted under: /api/customer-upsell-agent
...
// POST /api/customer-upsell-agent/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a landscaping upsell specialist. Your task is recommending maintenance plans and seasonal services. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/customer-upsell-agent/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/customer-upsell-agent/:id — fetch one
...
// DELETE /api/customer-upsell-agent/:id — discard a run
