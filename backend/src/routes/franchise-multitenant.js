// Auto-generated feature route: franchise-multitenant
// Domain: franchise SaaS configurator — Capability: provisioning white-label settings for franchise operators
// Mounted under: /api/franchise-multitenant
...
// POST /api/franchise-multitenant/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a franchise SaaS configurator. Your task is provisioning white-label settings for franchise operators. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/franchise-multitenant/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/franchise-multitenant/:id — fetch one
...
// DELETE /api/franchise-multitenant/:id — discard a run
