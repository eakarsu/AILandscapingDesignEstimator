...
// Route: /api/gap-crew-skill-matcher
// Feature: Crew Skill Matcher
// Description: (match crew expertise to project needs)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Crew Skill Matcher. (match crew expertise to project needs)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Crew Skill Matcher\nDescription: (match crew expertise to project needs)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['crew-skill-matcher', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'crew-skill-matcher', title: 'Crew Skill Matcher', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'crew-skill-matcher', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['crew-skill-matcher']);
      return res.json({ slug: 'crew-skill-matcher', items: r.rows || [] });
    }
    return res.json({ slug: 'crew-skill-matcher', items: [] });
  } catch (e) {
    return res.json({ slug: 'crew-skill-matcher', items: [], error: (e && e.message) || 'Server error' });
  }
});
