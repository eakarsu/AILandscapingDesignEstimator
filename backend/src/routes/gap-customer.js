...
// Route: /api/gap-customer
// Feature: Customer
// Description: Customer portal (status visibility, online payment)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Customer. Customer portal (status visibility, online payment)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    return res.json({ feature: 'customer', title: 'Customer', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'customer', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['customer']);
      return res.json({ slug: 'customer', items: r.rows || [] });
    }
    return res.json({ slug: 'customer', items: [] });
  } catch (e) {
    return res.json({ slug: 'customer', items: [], error: (e && e.message) || 'Server error' });
  }
});
