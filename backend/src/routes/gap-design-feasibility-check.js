...
// Route: /api/gap-design-feasibility-check
// Feature: Design Feasibility Check
// Description: (validate design against budget constraints)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Design Feasibility Check. (validate design against budget constraints)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Design Feasibility Check\nDescription: (validate design against budget constraints)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['design-feasibility-check', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'design-feasibility-check', title: 'Design Feasibility Check', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'design-feasibility-check', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['design-feasibility-check']);
      return res.json({ slug: 'design-feasibility-check', items: r.rows || [] });
    }
    return res.json({ slug: 'design-feasibility-check', items: [] });
  } catch (e) {
    return res.json({ slug: 'design-feasibility-check', items: [], error: (e && e.message) || 'Server error' });
  }
});
