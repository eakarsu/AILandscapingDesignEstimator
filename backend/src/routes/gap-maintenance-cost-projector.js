...
// Route: /api/gap-maintenance-cost-projector
// Feature: Maintenance Cost Projector
// Description: (estimate ongoing maintenance costs)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Maintenance Cost Projector. (estimate ongoing maintenance costs)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Maintenance Cost Projector\nDescription: (estimate ongoing maintenance costs)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['maintenance-cost-projector', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'maintenance-cost-projector', title: 'Maintenance Cost Projector', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'maintenance-cost-projector', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['maintenance-cost-projector']);
      return res.json({ slug: 'maintenance-cost-projector', items: r.rows || [] });
    }
    return res.json({ slug: 'maintenance-cost-projector', items: [] });
  } catch (e) {
    return res.json({ slug: 'maintenance-cost-projector', items: [], error: (e && e.message) || 'Server error' });
  }
});
