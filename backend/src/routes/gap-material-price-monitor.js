...
// Route: /api/gap-material-price-monitor
// Feature: Material Price Monitor
// Description: (flag inflation, suggest alternatives)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Material Price Monitor. (flag inflation, suggest alternatives)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Material Price Monitor\nDescription: (flag inflation, suggest alternatives)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['material-price-monitor', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'material-price-monitor', title: 'Material Price Monitor', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'material-price-monitor', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['material-price-monitor']);
      return res.json({ slug: 'material-price-monitor', items: r.rows || [] });
    }
    return res.json({ slug: 'material-price-monitor', items: [] });
  } catch (e) {
    return res.json({ slug: 'material-price-monitor', items: [], error: (e && e.message) || 'Server error' });
  }
});
