...
// Route: /api/gap-seasonal-demand-forecast
// Feature: Seasonal Demand Forecast
// Description: (predict project volume by quarter)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Seasonal Demand Forecast. (predict project volume by quarter)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Seasonal Demand Forecast\nDescription: (predict project volume by quarter)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['seasonal-demand-forecast', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'seasonal-demand-forecast', title: 'Seasonal Demand Forecast', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'seasonal-demand-forecast', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['seasonal-demand-forecast']);
      return res.json({ slug: 'seasonal-demand-forecast', items: r.rows || [] });
    }
    return res.json({ slug: 'seasonal-demand-forecast', items: [] });
  } catch (e) {
    return res.json({ slug: 'seasonal-demand-forecast', items: [], error: (e && e.message) || 'Server error' });
  }
});
