// Auto-generated feature route: material-price-aggregator
// Domain: landscaping materials buyer — Capability: aggregating supplier price feeds and suggesting bulk-buy alerts
// Mounted under: /api/material-price-aggregator
  ...
  return new Promise((resolve) => {
    ...
    const options = {
      ...
      headers: {
        ...
        'X-Title': 'Landscaping Design Estimator'
      }
    };
...
// POST /api/material-price-aggregator/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a landscaping materials buyer. Your task is aggregating supplier price feeds and suggesting bulk-buy alerts. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
    ...
    const record = {
      ...
      slug: 'material-price-aggregator',
      input: payload,
      output: parsed,
...
// GET /api/material-price-aggregator/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/material-price-aggregator/:id — fetch one
...
// DELETE /api/material-price-aggregator/:id — discard a run
