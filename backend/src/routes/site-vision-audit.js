// Auto-generated feature route: site-vision-audit
// Domain: landscape design vision auditor — Capability: generating design suggestions and cost estimates from site photos
// Mounted under: /api/site-vision-audit
...
// POST /api/site-vision-audit/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a landscape design vision auditor. Your task is generating design suggestions and cost estimates from site photos. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/site-vision-audit/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/site-vision-audit/:id — fetch one
...
// DELETE /api/site-vision-audit/:id — discard a run
