...
migrate_files() {
  ...
  for migration in "$API_DIR"/src/migrations/*.sql; do psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -f "$migration"; done
}
...
start_services() {
  ...
  CLIENT_URL="http://127.0.0.1:$FRONTEND_PORT"
  REACT_APP_API_URL="http://127.0.0.1:$BACKEND_PORT/api"
  REACT_APP_API_ORIGIN="http://127.0.0.1:$BACKEND_PORT"
  export BACKEND_PORT FRONTEND_PORT CLIENT_URL REACT_APP_API_URL REACT_APP_API_ORIGIN
  ...
  if node -e "const p=require('./$UI_DIR/package.json');process.exit(p.scripts&&p.scripts.dev?0:1)"; then
    npm --prefix "$UI_DIR" run dev -- --host "${HOST:-127.0.0.1}" --port "$FRONTEND_PORT" --strictPort &
  else
    BROWSER=none HOST="${HOST:-127.0.0.1}" PORT="$FRONTEND_PORT" npm --prefix "$UI_DIR" start &
  fi
  ui_pid=$!
